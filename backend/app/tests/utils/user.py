...
def user_authentication_headers(*, client: TestClient, email: str,
    ...
    r = client.post(f"{config.API_V1_STR}/login/access-token", data=data)
    response = r.json()
...
def authentication_token_from_email(*, client: TestClient, email: str,
    ...
    And create user first if it doesn't exist.
    """
    password = random_lower_string()
    user = crud.user.get_by_email(db, email=email)
    if not user:
        user_in_create = UserCreate(username=email, email=email,
                                    password=password)
